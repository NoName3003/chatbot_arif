...
class Master extends DBConnection
{
	...
	public function get_response()
	{
		...
		$sql = "SELECT r.response_message,q.id,q.question from `questions` q inner join `responses` r on q.response_id = r.id where q.question Like '%{$message}%' ";
		// $sql = "SELECT id,question from `questions` where question Like  '%{$message}%' ";
		$qry = $this->conn->query($sql);

		// $qry_q = $this->conn->query($sql_q);
		// if ($qry_q->num_rows > 0) {
		// 	$result = $qry_q->fetch_array();
		// 	$resp_q['message'] = $result['response_message'];
		// 	$resp_q['sql'] = $sql_q;
		// 	return json_encode($resp_q);
		// }
		if ($qry->num_rows > 0) {
			while ($result = $qry->fetch_array()) {
				...
				$resp['message_q'] = $result['question'];
				$resp['sql'] = $sql;
